...
export default function CaliforniaPage() {
  ...
  const fetchInventory = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/inventory/CA')
      if (response.ok) {
  ...
  const handleUploadSuccess = () => {
    setCurrentStep(3) // Go to order summary
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="flex-1 p-6">
        ...
        {currentStep === 2 && (
          <>
            {console.log('Debug - customerInfo before passing to upload:', customerInfo)}
            <UploadRequirements 
        ...
        {currentStep === 3 && (
          <OrderSummaryStep state="California" selectedAd={selectedAd} />
        )}
      </main>
